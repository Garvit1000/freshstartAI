...
router.post('/optimize-resume', upload.single('resume'), async (req, res) => {
  try {
    ...
    const { text: resumeText, formattingInfo } = await extractTextFromPDF(req.file.buffer, req.app.locals.analyzeDocumentStructureWithAdobe);
    
    if (!resumeText || resumeText.trim().length === 0) {
      return res.status(400).json({ 
        error: 'Could not extract text from the provided PDF. Please ensure it contains readable text.' 
      });
    }

    console.log("Extracted text from PDF, analyzing with ATS score calculations...");
    
    // First calculate ATS score for original resume to identify improvement areas
    const originalATSScore = await calculateATSScore(resumeText, req.body.jobDescription);
...
`;
    ...
    const transparencyMode = req.body.transparencyMode === 'true' || req.body.transparencyMode === true;
    
    // Now optimize the resume with this enhanced guidance
    const result = await optimizeResume(resumeText, req.body.jobDescription, formattingInfo, optimizationGuidance, transparencyMode);
    ...
    console.log("First 100 chars of optimized text:", finalOptimizedText.substring(0, 100));
    
    const skillGaps = await analyzeSkillGaps(resumeText, req.body.jobDescription);

    // Calculate ATS score for optimized resume
    const optimizedATSScore = await calculateATSScore(finalOptimizedText, req.body.jobDescription);
    ...
    const improvements = {
      ...
      formatting: optimizedATSScore.formattingScore - originalATSScore.formattingScore,
    ...
    modifyPDFWithFallback(req.file.buffer, finalOptimizedText, formattingInfo, template)
    ...
    res.status(500).json({
      ...
      details: {
        ...
        originalError: error.originalError ? error.originalError.message : error.message
      }
    });
  }
});
...
router.post('/update-resume', upload.single('resume'), async (req, res) => {
  try {
    ...
    const { text: originalText, formattingInfo } = await extractTextFromPDF(req.file.buffer, req.app.locals.analyzeDocumentStructureWithAdobe);
    
    // Log the content for debugging
    console.log('Custom text length:', customText.length);
    console.log('Custom text first 100 chars:', customText.substring(0, 100));
    console.log('Custom text last 100 chars:', customText.substring(customText.length - 100));
    console.log('Number of lines:', customText.split('\n').length);
    
    try {
      // Calculate ATS score for the custom text
      const customATSScore = await calculateATSScore(customText, req.body.jobDescription);
      ...
      try {
        ...
        const optimizedPdfBuffer = await modifyPDFWithFallback(req.file.buffer, customText, formattingInfo, template);
        
        if (!optimizedPdfBuffer || optimizedPdfBuffer.length < 1000) {
          console.warn('Warning: PDF buffer is suspiciously small:', optimizedPdfBuffer?.length);
        }
        ...
        console.log('Custom resume PDF generated successfully, size:', optimizedPdfBuffer.length);
        ...
        const fallbackBuffer = await modifyPDFWithFallback(req.file.buffer, customText, formattingInfo, template);
      ...
      res.status(500).json({
        error: 'Failed to process resume with edited text. PDF download will use the original format.',
...
router.get('/download-pdf', (req, res) => {
  try {
    ...
    if (Date.now() - pdfData.timestamp > 5 * 60 * 1000) {
      ...
      return res.status(404).json({ error: 'PDF has expired. Please generate a new one.' });
    }

    // If we have custom text but the PDF doesn't have it, try to regenerate
    if (pdfData.customText && !pdfData.hasCustomText && pdfData.originalBuffer) {
      ...
      try {
        modifyPDFWithFallback(pdfData.originalBuffer, pdfData.customText, pdfData.formattingInfo || {}, pdfData.template)
          .then(newBuffer => {
...
router.post('/generate-direct-pdf', upload.single('resume'), async (req, res) => {
  try {
    ...
    console.log(`Processing direct PDF with ${customText.length} characters of text`);
    ...
    const enhancedFormatting = {
      ...
      bulletCount: (customText.match(/•/g) || []).length + (customText.match(/^-\s/mg) || []).length,
    ...
    try {
      ...
      if (jobDescription) {
        ...
        const editedAtsScore = await calculateATSScore(customText, jobDescription);
        
        // Include both scores and calculate improvements
        if (originalAtsScore) {
          const improvements = {
            overall: editedAtsScore.overallScore - originalAtsScore.overallScore,
            keywords: editedAtsScore.keywordScore - originalAtsScore.keywordScore, 
            formatting: editedAtsScore.formattingScore - originalAtsScore.formattingScore,
            content: editedAtsScore.contentScore - originalAtsScore.contentScore
          };
      ...
      const pdfBuffer = await modifyPDFWithFallback(req.file.buffer, customText, enhancedFormatting, template);
      
      if (!pdfBuffer || pdfBuffer.length < 1000) {
        throw new Error(`Generated PDF appears to be invalid (size: ${pdfBuffer?.length} bytes)`);
      }
